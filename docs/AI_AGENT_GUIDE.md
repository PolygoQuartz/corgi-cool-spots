...
**スポット（spots.js）**
- 渋滞が顕著なエリアのスポットには `congestion: {factor(通常混雑), months?, weekendOnly?, peak:{from,to,factor}(激混み時間帯・現在時刻で切替), note}` を設定できる（該当条件の日だけ移動時間に係数が掛かり「⚠️渋滞・通常約N分」表示になる。出発地変更時の推定にも適用される）。リアルタイム渋滞ではなく「それらしい目安」を出すための静的係数（飼い主の方針）。
- 現行係数は公的データで校正済み（2026-08-04・夏土日午前の行き方向基準）: 湘南1.5（鎌倉R134実測: 20分区間が渋滞時60分・県資料）／伊豆1.6（沼津河川国道事務所の混雑実績等）／三浦1.3（三浦縦貫道Ⅱ期事後評価・県）／富士五湖1.25（国交省中央道WG＋NEXCO予測）／秩父1.25（NEXCO予測＋県道路公社）／アクアライン1.15（千葉県検討会の下り実測: 土曜朝+8分）／奥多摩1.1（定量データなし・低信頼）。連休・お盆はこれより悪化するが係数は通常週末基準。
