...
- 実測していない温度を実測値として書かない。素手で触った体感（yamlの `hand_feel`）は `groundTemps[].feel`（危険/アチアチ/人肌/ひんやり）として記録し、温度数値に変換しない。
- 過去の訪問記録は変更しない。

**スポット（spots.js）**
- 渋滞が顕著なエリアのスポットには `congestion: {factor, months?, weekendOnly?, note}` を設定できる（該当条件の日だけ移動時間に係数が掛かり「⚠️渋滞・通常約N分」表示になる。出発地変更時の推定にも適用される）。リアルタイム渋滞ではなく「それらしい目安」を出すための静的係数（飼い主の方針）。
...
飲食店・商業施設型スポットは閉業・移転・休業が発生する。**半年ごと（春・秋）を目安に全店の閉業チェックを行う**こと。

1. RESTAURANTS全件＋商業施設型スポット（pool カテゴリ・道の駅等）のリストを作る
2. 各店を「店名 エリア 閉店」で検索し、食べログの閉店マーク・Googleの閉業表示・閉店告知を根拠に判定する
3. 閉業確定・休業中（再開日未定含む）・実在や営業実態が確認できない店→**エントリと参照を削除**／移転→座標・noteを更新
4. **「実在」と「犬OK」の両方が確実な店だけを残す**（飼い主の方針: 確証がなければ削除が安全。再開・確認が取れたら再調査して復活させればよい）
5. 実施記録として確認日をコミットメッセージに残す（直近実施: 2026-08-04・全156件）
